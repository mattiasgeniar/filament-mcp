...
- **Audited.** Every tool call is recorded and attributed to the token that made it. Review a token's call history from the tokens page.
...
exists: list/get require an index or manage page, create requires a create or
manage page, and update requires an edit or manage page. `delete_*` tools are
