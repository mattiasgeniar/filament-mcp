...
The page inference is the **default**, not a hard gate: an explicit boolean for
an operation in the resource config overrides it. Set `'list' => true` to expose
`list_*` on a view-only resource that has no index page (handy for read-only,
...
3. **Filament resource surface** — by default tools are generated only for
   operations that have matching resource pages, and delete is an explicit opt-in.
   An explicit boolean per operation in the resource config overrides this default.
